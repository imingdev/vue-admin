...
<script type="text/javascript">
  ...
  export default{
    data(){
      return {
        media_message_id: this.$route.params.id,
    ...
    methods: {
      ...
      onSubmit(){
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.$http.post(url_update_media_message, {
              id: this.media_message_id,
      ...
      delete_message_data(index){
        let _id = this.message_data[index].id
        if (typeof _id === 'undefined') {
          ...
          this.$http.post(url_delete_media_message_sub_by_id, {id: _id})
            .then(() => {
              this.message_data.splice(index, 1)
              this.index = this.message_data.length - 1
              this.set_form_data()
      ...
      //获取数据
      get_media_message_data_by_id(){
        this.$http.get(url_media_message_by_id + "?id=" + this.media_message_id)
          .then(({data:{data, code, msg}}) => {
            let _message = data.message
            this.message_data = new Array(_message.length)
            for (let i = 0; i < _message.length; i++) {
              let _obj
              let _dm = _message[i]
              if (typeof _dm.id === 'undefined') {
                _obj = {
                  title: _dm.title,
                  descrip: _dm.descrip,
                  image: _dm.image,
                  ...
                  type: _dm.type,
                  content: _dm.content,
                  origin_url: _dm.origin_url,
                  urlContent: _dm.type === EVENT_URL ? _dm.content : null,
                  inputContent: _dm.type === EVENT_VIEW ? _dm.content : null
                }
              } else {
                _obj = {
                  id: _dm.id,
              ...
              this.message_data[i] = _obj
            }
            this.site_type_value = data.site
            this.set_form_data()
          })
      }
    },
    ...
    created() {
      this.get_media_message_data_by_id()
    }
  }
</script>
